...
interface ParentPhotoDialogProps {
  parent: StudParent | null;
  onClose: () => void;
  onSaved: (pId: number) => void;
}

// View/edit(rotate+resize)/save dialog for one parent's photo - verbatim adaptation of
// StudentPhotoDialog.tsx (same rotate/zoom/pan canvas editor, same 500KB JPEG-quality-stepping
// save), swapped to StudParentReader/p_id.
...
const ParentPhotoDialog = ({ parent, onClose, onSaved }: ParentPhotoDialogProps) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    StudParentReader.loadParentPhotoImage(accessToken, connection, parent.p_id).then((img) => {
      if (!cancelled) {
        setSourceImage(img);
  ...
  const handleSave = async () => {
    if (!parent || !sourceImage || !canvasRef.current) {
      return;
    }
    ...
    setIsSaving(true);
    const result = await StudParentReader.uploadParentPhoto(
      accessToken,
      connection,
      parent.p_id,
      blob,
    ...
    <dialog ref={dialogRef} className="modal" onClose={onClose}>
      <div className="modal-box">
        <h3 className="font-bold text-lg mb-4">
          {parent ? t.photoDialogTitle(`${parent.p_name} ${parent.p_surname ?? ""}`.trim()) : ""}
        </h3>
...
export default ParentPhotoDialog;
