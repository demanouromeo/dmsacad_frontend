...
interface StaffPhotoDialogProps {
  staff: Staff | null;
  onClose: () => void;
  onSaved: (staffId: number) => void;
}
...
// View/edit(rotate+resize)/save dialog for one staff member's photo - a single shared instance
// rendered once by StaffManager (native <dialog>, same modal/modal-box/modal-action/modal-backdrop
// pattern as TopBanner's dialogs), opened/closed by whether `staff` is set. Mirrors
// StudentPhotoDialog.tsx exactly.
const StaffPhotoDialog = ({ staff, onClose, onSaved }: StaffPhotoDialogProps) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    StaffReader.loadStaffPhotoImage(accessToken, connection, staff.staff_id).then((img) => {
      if (!cancelled) {
        setSourceImage(img);
  ...
  const handleSave = async () => {
    if (!staff || !sourceImage || !canvasRef.current) {
      return;
    }
    ...
    setIsSaving(true);
    const result = await StaffReader.uploadStaffPhoto(
      accessToken,
      connection,
      staff.staff_id,
      blob,
    ...
    <dialog ref={dialogRef} className="modal" onClose={onClose}>
      <div className="modal-box">
        <h3 className="font-bold text-lg mb-4">
          {staff ? t.photoDialogTitle(`${staff.name} ${staff.surname ?? ""}`.trim()) : ""}
        </h3>
...
export default StaffPhotoDialog;
